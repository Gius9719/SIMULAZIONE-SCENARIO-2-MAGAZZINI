...
# Input
input_path = '/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/FILE PER PYTHON/SCENARI/4/INPUT.xlsx'
statistiche_path = '/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/FILE PER PYTHON/SCENARI/4/LOGICA_STEP4.xlsx'

# Output
output_shipm_in_out_prod_path = f'/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/OUTPUT PYTHON/Simulazioni/SCENARI/SCENARIO 4/SIMULAZIONI/ANNO/{int(single_thr*100)}/sped_ingressi_uscite_prod.xlsx'
output_stock_path = f'/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/OUTPUT PYTHON/Simulazioni/SCENARI/SCENARIO 4/SIMULAZIONI/ANNO/{int(single_thr*100)}/scorte.xlsx'
output_shuttling_and_cap_path = f'/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/OUTPUT PYTHON/Simulazioni/SCENARI/SCENARIO 4/SIMULAZIONI/ANNO/{int(single_thr*100)}/shuttling_e_cap.xlsx'
output_shuttling_summary_path = f'/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/OUTPUT PYTHON/Simulazioni/SCENARI/SCENARIO 4/SIMULAZIONI/ANNO/{int(single_thr*100)}/info_shuttling.xlsx'
output_step4_case2_path = f'/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/OUTPUT PYTHON/Simulazioni/SCENARI/SCENARIO 4/SIMULAZIONI/ANNO/{int(single_thr*100)}/risultati_step4_caso2.xlsx'
output_word_path = f'/Users/giulianosmarrazzo/Desktop/TESI_COCA_COLA/OUTPUT PYTHON/Simulazioni/SCENARI/SCENARIO 4/SIMULAZIONI/ANNO/{int(single_thr*100)}/report.docx'
...
stat_dataset = pd.read_excel(statistiche_path, sheet_name='STATISTICHE_71')
...
# Filtra gli item per famiglie AA e A (runner)
...
# Inizializzazione delle soglia minime e massime di bilanciamento
min_balancing_thr_A= 0.20
min_balancing_thr_AA = 0.20
max_balancing_thr_A = single_thr
max_balancing_thr_AA = single_thr

# Contatori per tenere traccia delle occorrenze di overcapacity nei magazzini
no_overcap_count = 0
overcap_72_count = 0
overcap_71_count = 0
overcap_72_71_count = 0

# Definisco lo step_size per la determinazione di cov_day_star nello step 4
step_size = 0.1
...
total_cap_shutt_df = pd.DataFrame(0.0, index=['SHUTT CAPACITA TOTALI'], columns=date_list)
total_shutt_hub_df = pd.DataFrame(0.0, index=item_list, columns=date_list)

#  Creo i DF per le scorte provvisorie (temporary), quelle in seguito agli shuttling da fabbisogno di materiale (needs)
#  e infine quelle aggiornate in seguito agli shuttling per il rispetto della capacità (final)
...
balancing_thr_df = pd.DataFrame(0.0, index=['SOGLIA FINALE A', 'SOGLIA FINALE AA'], columns=date_list)

# Creo il DF per memorizzare gli shuttling totali effettivi per il caso 2 dello step 4
total_eff_shutt_case2 = pd.DataFrame(0.0, index=['SHUTT EFFETTIVI'], columns=date_list)
...
for current_date_idx, current_date in enumerate(date_list):
    ...
    # Riallocazione e conteggio delle spedizioni per popolare di DF ref_sped_uscita_dict e out_shipm_dict_df
    ...
    if next_date:
        ...
        next_date_out_shipm = out_shipm_dataset[out_shipm_dataset['CurrLoadSt'] == pd.to_datetime(next_date, dayfirst=True)]
    ...
    # si disponga per ciascun item, dello stocknecessario a soddisfare le uscite del giorno successivo).
    ...
    for warehouse in warehouses:
        ...
        if next_date:
            if out_shipm_code_dict[warehouse][next_date]:
                next_date_filtered_out = out_dataset[out_dataset['Shipment'].isin(out_shipm_code_dict[warehouse][next_date])]
    ...
    for item in item_list:
        for warehouse in warehouses:
            ...
            if current_date_idx == 0:  # Primo Giorno
                ...
                else:  # Magazzino 6371 (no prod_plt)
                    temporary_stock_dict_df[warehouse].at[item, current_date] = (
        ...
        if next_date:
            ...
            # CASO 1: Scorta provvisoria di 71 al giorno corrente >= out_plt da 71 giorno successivo e Scorta provvisoria di 72 al giorno corrente >= out_plt da 72 giorno successivo
            if (temporary_stock_dict_df[6372].at[item, current_date] - out_item_dict_df[6372].at[item, next_date] >= 0 and
                temporary_stock_dict_df[6371].at[item, current_date] - out_item_dict_df[6371].at[item, next_date] >= 0):
                needs_shutt_stock_dict_df[6372].at[item, current_date] += 0
                needs_shutt_stock_dict_df[6371].at[item, current_date] += 0

            # CASO 2: Scorta provvisoria di 72 al giorno corrente >= out_plt da 72 giorno successivo e Scorta provvisoria di 71 al giorno corrente < out_plt da 71 giorno successivo
            elif (temporary_stock_dict_df[6372].at[item, current_date] - out_item_dict_df[6372].at[item, next_date] >= 0 and
                  temporary_stock_dict_df[6371].at[item, current_date] - out_item_dict_df[6371].at[item, next_date] < 0):
                needs_shutt_amount = np.ceil(out_item_dict_df[6371].at[item, next_date] - temporary_stock_dict_df[6371].at[item, current_date])
            ...
            # CASO 3: Scorta provvisoria di 72 al giorno corrente < out_plt da 72 giorno successivo e Scorta provvisoria di 71 al giorno corrente >= out_plt da 71 giorno successivo
            elif (temporary_stock_dict_df[6372].at[item, current_date] - out_item_dict_df[6372].at[item, next_date] < 0 and
                  temporary_stock_dict_df[6371].at[item, current_date] - out_item_dict_df[6371].at[item, next_date] >= 0):
            ...
            # CASO 4: Scorte provvisorie per entrambi i magazzini al giorno corrente < uscite da entrambi i magazzini giorno successivo
                ...
                print(f" Scorte insufficienti a coprire le uscite del giorno successivo per ITEM {item} in entrambi i magazzini alla data {current_date}:")
        ...
        # Logica degli shuttling da fabbisogno per il 31/12/3023 (non essendo più disponibile il giorno successivo)
        else:

            # CASO 1: Scorte >= 0 in entrambi i magazzini
            if (temporary_stock_dict_df[6372].at[item, current_date] >= 0 and
                temporary_stock_dict_df[6371].at[item, current_date] >= 0):
                needs_shutt_stock_dict_df[6372].at[item, current_date] += 0
                needs_shutt_stock_dict_df[6371].at[item, current_date] += 0

            # CASO 2: Scorta >= 0 in 6372 e negativa in 6371
            elif (temporary_stock_dict_df[6372].at[item, current_date] >= 0 and
                  temporary_stock_dict_df[6371].at[item, current_date] < 0):
                needs_shutt_amount = np.ceil(-temporary_stock_dict_df[6371].at[item, current_date])
                needs_shutt_dict_df[6372][6371].at[item, current_date] = needs_shutt_amount
                needs_shutt_stock_dict_df[6372].at[item, current_date] -= needs_shutt_amount
                needs_shutt_stock_dict_df[6371].at[item, current_date] += needs_shutt_amount

            # CASO 3: Scorta negativa in 6372 e >= 0 in 6371
            elif (temporary_stock_dict_df[6372].at[item, current_date] < 0 and
                  temporary_stock_dict_df[6371].at[item, current_date] >= 0):
            ...
            # CASO 4: Scorte negative in entrambi i magazzini
            else:
                print(f" Scorte negative per ITEM {item} in entrambi i magazzini alla data {current_date}")
        ...
        # Verifico che alla data corrente tutti e 2 i magazzini abbiano una scorta in seguito agli shuttling da fabbisofno
        ...
        for warehouse in warehouses:
            if next_date:
                if needs_shutt_stock_dict_df[warehouse].at[item, current_date] - out_item_dict_df[warehouse].at[item, next_date] >= 0:
    ...
    # CASO 1: Nessuna eccedenza in entrambi i magazzini
    if overcap_dict_df[6372].at['OVERCAP', current_date] == 0 and overcap_dict_df[6371].at['OVERCAP', current_date] == 0:
        print(f"In nessuno dei magazzini si supera la capacità alla data {current_date}")
        no_overcap_count += 1

    # CASO 2: Eccedenza solo in 6372
    elif overcap_dict_df[6372].at['OVERCAP', current_date] > 0 and overcap_dict_df[6371].at['OVERCAP', current_date] == 0:
        print(f"Si super la capacità in 6372 alla data {current_date}")
        overcap72 = overcap_dict_df[6372].at['OVERCAP', current_date]
        overcap_72_count += 1
        cap_shutt_72_71 = overcap72
        ...
        print("FASE 1: VERIFICA AMMISSIBILITA' SOGLIA DI BILANCIAMENTO PER SHUTTLING DA 72 A 71")
        info_movable_items = {}
        ...
        while True:
            ...
            for item in movable_items:
                ...
                if next_date:
                    effective_stock72 = needs_shutt_stock_dict_df[6372].at[item, current_date] - out_item_dict_df[6372].at[item, next_date]
                    effective_stock71 = needs_shutt_stock_dict_df[6371].at[item, current_date] - out_item_dict_df[6371].at[item, next_date]
                else:
                    effective_stock72 = needs_shutt_stock_dict_df[6372].at[item, current_date]
                    effective_stock71 = needs_shutt_stock_dict_df[6371].at[item, current_date]
                potential_shutt = max(0, effective_stock72 - np.ceil((effective_stock72 + effective_stock71) * alpha))
            ...
            if total_potential_shutt >= cap_shutt_72_71:
                ...
                print(f"Soglia di bilanciamento ammissibile per shuttling da 72 a 71:")
        ...
        total_eff_shutt_case2.at['SHUTT EFFETTIVI', current_date] = sum_total_eff_shutt
        # Ripartizione degli shuttling secondo cov_day_star percentuale
        for item, data in moved_items_dict.items():
            # Aggiornamento dei DF (CONTROLLA)
        ...
        total_cap_shutt_df.loc['SHUTT CAPACITA TOTALI', current_date] = cap_shutt_dict_df[6372][6371][current_date].sum()
        ...
        sheet_name = f"{current_date}_{cov_day_star:.2f}".replace('/', '-')
        # Scrivo nel file Excel
    ...
    # CASO 3: Eccedenza solo in 6371
    elif overcap_dict_df[6372].at['OVERCAP', current_date] == 0 and overcap_dict_df[6371].at['OVERCAP', current_date] > 0:
        print(f"Si super la capacità in 6371 alla data {current_date}")
        overcap_71_count += 1

    # CASO 4: Eccedenze in entrambi i magazzini
    elif overcap_dict_df[6372].at['OVERCAP', current_date] > 0 and overcap_dict_df[6371].at['OVERCAP', current_date] > 0:
        print(f"Errore: Entrambi i magazzini hanno superato la capacità alla data {current_date}")
    ...
    for item in item_list:
        for warehouse in warehouses:
            if next_date:
                if final_stock_dict_df[warehouse].at[item, current_date] - out_item_dict_df[warehouse].at[item, next_date] >= 0:
                    cap_check_passed += 1
                else:
                    cap_check_failed += 1
...
overcap_date_72 = [date for date in date_list if overcap_dict_df[6372].at['OVERCAP', date] > 0]
...
sum_total_shutt_type = {
    ...
    'SHUTT TOT HUB': total_shutt_hub_df.sum().sum()
}
...
shuttling_by_family_df.index.name = "FAMIGLIA"  # Nome dell'indice

## STAMPE ##

print(f"Occorrenze overcap di 72: {overcap_72_count}")
print(f"Occorrenze overcap di 71: {overcap_71_count}")
...
if overcap_71_count > 0:
    sum_overcap71 = overcap_dict_df[6371].loc['OVERCAP'].sum()
    mean_overcap71 = sum_overcap71 / overcap_71_count
    print(f"Overcap media di 72: {mean_overcap71:.0f}")
...
# Scorta media di runner A e AA per i due magazzini
def calculate_mean_stock(final_stock_dict, items_r_AA, items_r_A, date_list):
    ...
    for warehouse, df_scorte in final_stock_dict.items():
        ...
        daily_AA_stock = df_scorte.loc[items_r_AA].sum(axis=0)
        daily_A_stock = df_scorte.loc[items_r_A].sum(axis=0)
...
doc.add_paragraph(f"Occorrenze overcap di 71: {overcap_71_count}")
...
doc.add_heading('Livello medio di scorte per categorie A e AA', level=2)
# Tabella
...
print(f"Report Word salvato in: {output_word_path}")
